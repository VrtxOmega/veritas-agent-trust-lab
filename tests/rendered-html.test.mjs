...
test("server renders the finished public lab", async () => {
  ...
  assert.match(visibleHtml, /About five minutes on phone or desktop/);
  assert.match(visibleHtml, /manually send the private email commitment/);
  assert.match(visibleHtml, /Download pre-reveal commitment/);
...
test("public source preserves commercial and assurance boundaries", async () => {
  ...
  assert.match(page, /veritas-trust-lab-pre-reveal-commitment\.json/);
  assert.match(page, /candidate corpus first—not the canonical challenge/);
  assert.match(page, /template=adopter-report\.yml/);
